...
let debug = require('debug')('digs-broker:digs-broker');
...
function _notifyPing(client) {
  debug('<digs.broker>: <%s#%s> is alive', client.constructor.name, client.id);
}
...
function handler(client) {
  ...
  client.on('connect', function (packet) {
    ...
    debug('<digs.broker>: <%s#%s> connected', client.constructor.name,
      client.id);
  ...
  client.on('subscribe', function (packet) {
    debug('<digs.broker>: <%s#%s> subscribing to topic(s):',
  ...
  client.on('publish', function (packet) {
    debug('<digs.broker>: <%s#%s> published topic "%s" w/ payload',
  ...
  client.on('disconnect', function () {
    debug('<digs.broker>: <%s#%s> disconnected', client.constructor.name,
      client.id);
    end(this);
  });

  client.on('close', function () {
    debug('<digs.broker>: deleted reference to <%s#%s>',
      client.constructor.name, client.id);
    delete this.clients[client.id];
  }.bind(this));

  client.on('error', function (e) {
    debug('<digs.broker>: received error from <%s#%s>', this.constructor.name,
      this.id, e);
    end(this);
  });
}
...
function createServer(opts) {
  ...
  debug('<digs.broker>: Instantiated local broker w/ opts:', opts);
  broker.listen(opts.port, opts.host, function () {
    let address = this.address();
    debug('<digs.broker>: Local broker listening on %s:%s', address.address,
      address.port);
  });
  return broker;
}
